...
const HyperAaugeI = ({ navigation }) => {
  ...
  const handleSave = () => {
    ...
    const userRef = ref(db, `assessmentHistory/${userId}`);
    ...
    push(userRef, newAssessmentEntry)
      .then(() => {
        ...
        if (sumOfNum >= 2) {
          alertMessage =
            "น้ำตาลในเลือดคุณสูงเกินไปสามารถเช็คระดับน้ำตาลในเลือดของคุณได้เลย";
          ...
          alertMessage = "คุณปกติ";
          Alert.alert("ผลแบบประเมินของคุณ", alertMessage, [
            {
              text: "ไปหน้า Chatusers",
              onPress: () => navigation.replace("Chatusers"),
            },
          ]);
        }
      })
